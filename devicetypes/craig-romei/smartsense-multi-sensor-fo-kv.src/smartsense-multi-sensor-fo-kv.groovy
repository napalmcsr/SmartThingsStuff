...
metadata {
	definition(name: "SmartSense Multi Sensor fo KV", namespace: "Craig.Romei", author: "Craig Romei") {
...
def parse(String description) {
	...
	if (!maps[0]) {
        ...
        if (description?.startsWith('temperature: ')) {
        	map.value = Double.parseDouble(description.split(": ")[1])
             }
		if (tempOffset) {
			map.value = map.value + tempOffset.toBigDecimal()
		}
        map.value = convertTemperature(map.value)
        log.debug "temp: ${map.value}"
		map.descriptionText = "{{ device.displayName }} was ${map.value}°${temperatureScale}"
...
def configure() {
	...
	configCmds += zigbee.batteryConfig() +
			zigbee.temperatureConfig(30, 60) +
